...
# Crear app Flask para health checks y triggers externos
app = Flask(__name__)

# Añade esto al inicio de tu aplicación Flask
@app.before_request
def before_request():
    """Log para requests de cron-job.org"""
    if request.path == '/trigger/promises':
        print(f"⏰ Trigger recibido de: {request.remote_addr} - {request.user_agent}")

# Mejora el endpoint de trigger
@app.route('/trigger/promises', methods=['GET', 'POST'])
def trigger_promises():
    """Endpoint para que servicios externos activen el envío de promesas"""
    try:
        print("🔔 Trigger de promesas recibido - Iniciando envío...")
        
        if bot_instance is None:
            error_msg = "❌ Bot no inicializado"
            print(error_msg)
            return {"status": "error", "message": error_msg}, 500
        
        # Ejecutar en un hilo asíncrono
        result = asyncio.run(bot_instance.enviar_promesas_diarias())
        
        print(f"✅ Envío completado: {result}")
        return result, 200
        
    except Exception as e:
        error_msg = f"❌ Error en trigger: {str(e)}"
        print(error_msg)
        return {"status": "error", "message": error_msg}, 500

@app.route('/status/promises', methods=['GET'])
def status_promises():
    """Endpoint para ver el estado de las suscripciones"""
    return {
        "status": "success",
...
# Almacenamiento global del bot para acceso desde Flask
bot_instance = None
...
# Almacenamiento de usuarios suscritos
subscribed_users = set()
# Para evitar enviar múltiples promesas el mismo día
last_sent_date = {}
...
class ClassBot:
    def __init__(self, token):
        ...
        # Registrar esta instancia globalmente para acceso desde Flask
        global bot_instance
        bot_instance = self

    async def enviar_promesas_diarias(self, context: ContextTypes.DEFAULT_TYPE = None):
        """Enviar promesas bíblicas a todos los usuarios suscritos"""
        if not subscribed_users:
            print("⏰ No hay usuarios suscritos para enviar promesas")
            return {"status": "success", "message": "No hay usuarios suscritos", "count": 0}
        
        today = datetime.now().date()
        
        # Verificar si ya enviamos promesas hoy
        if last_sent_date.get('global') == today:
            print("⏰ Ya se enviaron promesas hoy")
            return {"status": "success", "message": "Ya se enviaron promesas hoy", "count": 0}
        
        promesa = random.choice(BIBLE_PROMISES)
        mensaje = f"☀️ Buenos días! 📖 Promesa Bíblica del Día (7:00 AM):\n\n\"{promesa['text']}\"\n- {promesa['reference']}\n\n¡Que tengas un día bendecido! 🙏"
        ...
        for user_id in list(subscribed_users):
            try:
                # Verificar si este usuario ya recibió promesa hoy
                if last_sent_date.get(user_id) != today:
                    await self.application.bot.send_message(chat_id=user_id, text=mensaje)
                    last_sent_date[user_id] = today
                    success_count += 1
                    print(f"✅ Promesa enviada a usuario {user_id}")
                else:
                    print(f"⏰ Usuario {user_id} ya recibió promesa hoy")
                ...
                if "bot was blocked" in str(e).lower() or "chat not found" in str(e).lower():
                    ...
                    print(f"⚠️ Usuario {user_id} eliminado de suscripciones")
        
        last_sent_date['global'] = today
        print(f"📊 Resumen: {success_count} enviadas, {error_count} errores")
        
        return {
            "status": "success", 
            "message": f"Promesas enviadas a {success_count} usuarios",
    ...
    async def promesa_diaria(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if user_id in subscribed_users:
            keyboard = [
                [InlineKeyboardButton("✅ Mantener suscripción", callback_data="promesa_keep")],
                [InlineKeyboardButton("📖 Recibir promesa ahora", callback_data="promesa_now")],
                [InlineKeyboardButton("❌ Cancelar suscripción", callback_data="promesa_cancel")]
            ]
            reply_markup = InlineKeyboardMarkup(keyboard)
            
            await update.message.reply_text(
                "📖 Ya estás suscrito a la Promesa Bíblica Diaria.\n\n"
                "✅ Recibirás automáticamente una promesa cada día a las 7:00 AM (hora de Cuba).\n\n"
                "¿Qué deseas hacer?",
            ...
            keyboard = [
                ...
                [InlineKeyboardButton("📖 Solo recibir una ahora", callback_data="promesa_now_only")],
                [InlineKeyboardButton("❌ No, gracias", callback_data="promesa_decline")]
            ]
            reply_markup = InlineKeyboardMarkup(keyboard)
            
            await update.message.reply_text(
                "📖 ¿Te gustaría recibir una promesa bíblica cada día a las 7:00 AM para fortalecer tu fe?\n\n"
                "✅ Suscríbete y recibirás automáticamente versículos inspiradores cada mañana.\n\n"
    ...
    async def promesa_button(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if action == "subscribe":
            ...
            await query.edit_message_text(
                ...
                "📅 A partir de ahora recibirás automáticamente:\n"
                "• Una promesa bíblica cada día\n"
                "• Siempre a las 7:00 AM (hora de Cuba)\n"
                "• Mensajes inspiradores para empezar tu día\n\n"
                "¡Dios bendiga tus mañanas! 🌅📖\n\n"
                "Puedes cancelar en cualquier momento con /promesa"
            )
            ...
            if success:
                await query.edit_message_text(
                    "📖 Aquí tienes tu promesa bíblica.\n\n"
                    "✅ Recuerda que estás suscrito y recibirás otra automáticamente mañana a las 7:00 AM."
                )
            else:
                await query.edit_message_text("❌ Error al enviar la promesa. Intenta nuevamente.")
            
        elif action == "now_only":
            # Enviar promesa inmediata sin suscribir
            success = await self.send_promise_now(user_id)
            if success:
                await query.edit_message_text(
                    "📖 Aquí tienes tu promesa bíblica.\n\n"
                    "💡 Si quieres recibir promesas automáticamente cada día a las 7:00 AM, "
                    "usa /promesa y selecciona suscribirte."
                )
            else:
                await query.edit_message_text("❌ Error al enviar la promesa. Intenta nuevamente.")
            ...
            await query.edit_message_text(
                "❌ No te preocupes. Si cambias de opinión, siempre puedes suscribirte después "
                "con el comando /promesa.\n\n¡Que tengas un día bendecido! 🙏"
            )
            
        elif action == "keep":
            await query.edit_message_text(
                "✅ Perfecto, seguirás recibiendo la Promesa Bíblica Diaria cada mañana a las 7:00 AM.\n\n"
                "¡Dios bendiga tus días! 📖🌅"
            )
            ...
            await query.edit_message_text(
                "❌ Has cancelado tu suscripción a la Promesa Bíblica Diaria.\n\n"
...
• /promesa - Recibir promesas bíblicas diarias (7:00 AM)
• /help - Obtener ayuda
...
📖 Ofrezco promesas bíblicas diarias a las 7:00 AM
...
/normas - Ver reglas del grupo
/promesa - Suscribirse a promesas bíblicas diarias (7:00 AM)
...
- Recibir promesas bíblicas diarias a las 7:00 AM
        """
...
# Endpoint Flask para trigger externo
@app.route('/trigger/promises', methods=['GET', 'POST'])
def trigger_promises():
    """Endpoint para que servicios externos activen el envío de promesas"""
    try:
        if bot_instance is None:
            return {"status": "error", "message": "Bot no inicializado"}, 500
...
async def main():
    ...
    print("⏰ Sistema de promesas: Trigger externo a las 7:00 AM")
    
    # Iniciar servidor Flask en segundo plano
    flask_thread = threading.Thread(target=run_flask, daemon=True)
    flask_thread.start()
    print("🌐 Servidor Flask iniciado para health checks (puerto 5000)")
    print("🔗 Endpoint para promesas: /trigger/promises")
    print("📊 Endpoint para estado: /status/promises")
    ...
    try:
        ...
        print("📖 Las promesas se enviarán cuando se llame a /trigger/promises")
